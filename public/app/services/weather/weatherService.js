...
angular.module('myApp').service('WeatherService', function ($http, $q, UrlService, baseApi) {
	...
	// Getting the weather API documentation URL and a promise to know when it's loaded.
	docUrlReceived = UrlService.weatherUrl.then(function (url) {
		docUrl = url;
	});

	// When the documentation is received, getting the weathers resource URL and a promise to know when it's loaded.
...
//Process each property
		...
		clause = addStringLike('location', searchText);
		...
		clause = addNumberLike('month', searchText);

		if (clause != null)
			clauses.push(clause);

		clause = addNumberLike('day', searchText);
		...
		clause = addNumberLike('relHum', searchText);
		...
		clause = addNumberLike('hmdx', searchText);
	...
	WeatherService.getCount =  safeCall(function (opts) {
		opts = opts || {};
		opts.count = true;		
	...
	WeatherService.getList = safeCall(function (opts) {
		opts = opts || {};
		var q = buildBaucisQuery(opts);
		return $http.get(resourceUrl + q);
	});

	WeatherService.getListAsCsv = safeCall(function () {
		return $http({
			method: 'GET', 
			url: resourceUrl, 
	...
	WeatherService.getFileAsCsv = safeCall(function () {
		return $http({
			method: 'GET', 
	...
	WeatherService.getFileAsXml = safeCall(function () {
		return $http({
			method: 'GET', 
	...
	WeatherService.getFileAsXlsx = safeCall(function () {
		return $http({
			method: 'GET', 
	...
	WeatherService.getToEdit = safeCall(function (id) {
		return WeatherService.get(resourceUrl + '/' + id );
	});
	...
	WeatherService.add = safeCall(function (item) {
	...
	WeatherService.delete = safeCall(function (id) {
		return $http.delete(resourceUrl + '/' + id);
	});

	WeatherService.deleteMany = safeCall(function (ids) {
		var msg = { 
			'className' : 'weather',
			'ids'		: ids
		};	
	...
	WeatherService.deleteAll = safeCall(function (opts) {
		var msg = { 
			'className' : 'weather',
