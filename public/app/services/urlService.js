...
angular.module('myApp').service('UrlService', function (baseUrl, documentationUrl, $http, $rootScope) {
	...
	UrlService.olympicMedalUrl = apiDoc.then(function (apiDocumentation) {
		return getApi(apiDocumentation.data, 'olympicMedals');
	});

	UrlService.weatherUrl = apiDoc.then(function (apiDocumentation) {
		return getApi(apiDocumentation.data, 'weathers');
	});

	UrlService.exoplanetUrl = apiDoc.then(function (apiDocumentation) {
		return getApi(apiDocumentation.data, 'exoplanets');
	});
