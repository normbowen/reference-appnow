...
angular.module('myApp').controller('ListCountryController', function ($http, $scope, $location, $q, $timeout, NavigationService, CountryService) {
	$scope.dataList = [];
	...
	$scope.edit = function (obj) {
		$location.path('/country/edit/' + obj._id);
	};
	
	$scope.delete = function (obj) {
		$location.path('/country/delete/' + obj._id);
	};
	...
	$scope.importData = function () {
		...
		$location.path('/import/country');		
	};

	$scope.exportAsCsv = function () { 
		$scope.dataReceived = false;	
		CountryService.getFileAsCsv()
		.then(function(httpResponse) {
			sendFile(httpResponse, "countries.csv", "text/csv");
		})
		.finally(function() {
			$scope.dataReceived = true;
		});
	...
	$scope.exportAsXml = function () { 
		$scope.dataReceived = false;	
		CountryService.getFileAsXml().then(function(httpResponse) {
			sendFile(httpResponse, "countries.xml", "text/xml");
		})
		.finally(function() {
			$scope.dataReceived = true;
		});
	...
	$scope.exportAsXlsx = function () {
		$scope.dataReceived = false;	
		CountryService.getFileAsXlsx().then(function(httpResponse) {
			sendFile(httpResponse, "countries.xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
		})
		.finally(function() {
			$scope.dataReceived = true;
		});
	...
	function sendFile(httpResponse, fileName, mime) {
		...
		var filename = headers["x-filename"] || fileName || "countries.csv";
