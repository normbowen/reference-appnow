...
angular.module('myApp').controller('ListExoplanetController', function ($http, $scope, $location, $q, $timeout, NavigationService, ExoplanetService) {
	$scope.dataList = [];
	...
	$scope.deleteSelected = function() {
		...
		ExoplanetService.deleteMany(ids)
		//.catch(function(err) {
		//})
		.finally(function(f) {
			$timeout(function(){
	...
	$scope.exportAsCsv = function () { 
		$scope.dataReceived = false;	
		ExoplanetService.getFileAsCsv()
		.then(function(httpResponse) {
			sendFile(httpResponse, "exoplanets.csv", "text/csv");
		})
		.finally(function() {
			$scope.dataReceived = true;
		});
	...
	$scope.exportAsXml = function () { 
		$scope.dataReceived = false;	
		ExoplanetService.getFileAsXml().then(function(httpResponse) {
			sendFile(httpResponse, "exoplanets.xml", "text/xml");
		})
		.finally(function() {
			$scope.dataReceived = true;
		});
	...
	$scope.exportAsXlsx = function () {
		$scope.dataReceived = false;	
		ExoplanetService.getFileAsXlsx().then(function(httpResponse) {
			sendFile(httpResponse, "exoplanets.xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
		})
		.finally(function() {
			$scope.dataReceived = true;
		});
	...
	function sendFile(httpResponse, fileName, mime) {
		...
		var filename = headers["x-filename"] || fileName || "exoplanets.csv";
